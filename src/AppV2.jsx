...
function AppV2() {
  ...
  // ── Legendas com IA (AppV2) ────────────────────────────────────────────────
  const [showSubtitleModal, setShowSubtitleModal] = useState(false);
  const [subtitleVideoId,   setSubtitleVideoId]   = useState(null);
  const [subtitleApiKey,    setSubtitleApiKey]     = useState(() => { try { return localStorage.getItem('groq_api_key') || ''; } catch { return ''; } });
  const [subtitleLang,      setSubtitleLang]       = useState('pt');
  ...
  // ── Gerar Legendas com IA (Groq Whisper) ──────────────────────────────────
  const generateSubtitles = async (videoId) => {
    const vid = videos.find(v => v.id === videoId);
    if (!vid?.videoEl || !vid.audioBuffer) {
      setSubtitleError('Este vídeo não possui faixa de áudio detectável.');
      return;
    }
    if (!subtitleApiKey.trim()) {
      setSubtitleError('Insira sua Groq API Key.');
      return;
    }
    setSubtitleError('');
    ...
    try {
      // Salvar API key no localStorage para reutilização
      try { localStorage.setItem('groq_api_key', subtitleApiKey.trim()); } catch {}

      // ── 1. Extrair áudio como WAV mono 16kHz via OfflineAudioContext ────────
      setSubtitleProgress(15);
      const srcBuf = vid.audioBuffer; // AudioBuffer já decodificado
      const targetSR = 16000;
      const numFrames = Math.ceil(srcBuf.duration * targetSR);
      ...
      setSubtitleProgress(50);

      // ── 3. Enviar ao Groq Whisper ────────────────────────────────────────────
      const formData = new FormData();
      formData.append('file', wavBlob, 'audio.wav');
      formData.append('model', 'whisper-large-v3');
      formData.append('response_format', 'verbose_json');
      formData.append('timestamp_granularities[]', 'segment');
      if (subtitleLang !== 'auto') formData.append('language', subtitleLang);

      const resp = await fetch('https://api.groq.com/openai/v1/audio/transcriptions', {
        method: 'POST',
        headers: { Authorization: `Bearer ${subtitleApiKey.trim()}` },
        body: formData,
      });
      setSubtitleProgress(80);

      if (!resp.ok) {
        const errJson = await resp.json().catch(() => ({}));
        throw new Error(errJson?.error?.message || `Groq API error ${resp.status}`);
      }
      const result = await resp.json();
      const segments = result.segments || [];
      if (!segments.length) throw new Error('Nenhum segmento de fala detectado no áudio.');
      setSubtitleProgress(92);

      // ── 4. Injetar no array lyrics com timestamps ajustados ao offset do vídeo
      const vidOffset = vid.start || 0; // posição do vídeo na timeline
      const trimSt    = vid.trimStart || 0;
      const speedFactor = vid.vidSpeed ?? 1;
      const newLyrics = segments.map((seg) => {
        const segStart = (seg.start - trimSt) / speedFactor + vidOffset;
        const segEnd   = (seg.end   - trimSt) / speedFactor + vidOffset;
        return {
          id:        Date.now() + Math.random(),
          text:      seg.text.trim(),
          start:     Math.max(vidOffset, parseFloat(segStart.toFixed(3))),
          end:       parseFloat(Math.max(segStart + 0.5, segEnd).toFixed(3)),
          x:         canvasRef.current ? canvasRef.current.width  / 2 : 360,
          y:         canvasRef.current ? canvasRef.current.height * 0.82 : 1049,
          fontSize:  fontSizeRef?.current  || fontSize  || 42,
          fontFamily: fontFamilyRef?.current || fontFamily || 'Bebas Neue',
          color:     textColorRef?.current  || textColor  || '#ffffff',
          shadowEnabled: true, shadowBlur: 12, shadowColor: 'rgba(0,0,0,0.9)',
          animType:  'none',
        };
      });
      pushHistory();
      ...
      setSubtitleError(err.message || 'Erro desconhecido.');
      setSubtitleLoading(false);
      setSubtitleProgress(0);
    }
  };
...
<div style={{height:1,background:'rgba(255,255,255,0.06)',margin:'4px 0'}} />
          ...
          {(activeImageId || activeVideoId) && (() => {
            ...
            return (
              <div style={{ padding: '12px 18px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                {isVid && (
                  <button
                    onClick={() => { setSubtitleVideoId(sel.id); setSubtitleError(''); setShowSubtitleModal(true); }}
                    style={{
                      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                      width: '100%', padding: '10px 0',
                      background: 'linear-gradient(135deg,rgba(139,92,246,0.18),rgba(0,191,255,0.12))',
                      border: '1px solid rgba(139,92,246,0.4)',
                      borderRadius: 12, cursor: 'pointer',
                      fontSize: 12, fontWeight: 700, color: '#c4b5fd',
                      transition: 'all 0.15s',
                    }}
                    onMouseEnter={e => e.currentTarget.style.background = 'linear-gradient(135deg,rgba(139,92,246,0.3),rgba(0,191,255,0.2))'}
                    onMouseLeave={e => e.currentTarget.style.background = 'linear-gradient(135deg,rgba(139,92,246,0.18),rgba(0,191,255,0.12))'}
                  >
                    <span style={{ fontSize: 16 }}>🎤</span>
                    Gerar Legendas com IA
                    <span style={{ fontSize: 10, background: 'rgba(139,92,246,0.25)', border: '1px solid rgba(139,92,246,0.5)', borderRadius: 999, padding: '2px 7px', color: '#a78bfa' }}>Whisper</span>
                  </button>
                )}
  ...
  if (activeVideoId === v.id) setActiveVideoId(null);
      ...
      {/* ══ MODAL LEGENDAS COM IA ═══════════════════════════════════════════════ */}
      {showSubtitleModal && createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 99999, background: 'rgba(0,0,0,0.82)', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)' }}
          onClick={e => { if (!subtitleLoading) setShowSubtitleModal(false); }}>
          <div onClick={e => e.stopPropagation()} style={{
            background: 'linear-gradient(180deg,#0f172a 0%,#0a0f1a 100%)',
            border: '1px solid rgba(139,92,246,0.4)', borderRadius: 24,
            padding: '32px 28px', width: 440, maxWidth: '94vw',
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg,#7b2ff7,#00BFFF)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>🎤</div>
                <div>
                  <div style={{ fontSize: 15, fontWeight: 800, color: '#fff' }}>Gerar Legendas com IA</div>
                  <div style={{ fontSize: 11, color: '#555' }}>Groq Whisper Large v3</div>
                </div>
              </div>
              {!subtitleLoading && (
                <button onClick={() => setShowSubtitleModal(false)} style={{ background: 'none', border: 'none', color: '#555', cursor: 'pointer', fontSize: 18, lineHeight: 1 }}>✕</button>
              )}
            </div>

            {/* API Key */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 11, color: '#a78bfa', fontWeight: 700 }}>Groq API Key</label>
              <input
                type="password"
                ...
                style={{ background: '#111', border: '1px solid rgba(139,92,246,0.35)', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#f0f0f0', outline: 'none', fontFamily: 'monospace' }}
              />
              <div style={{ fontSize: 10, color: '#444', display: 'flex', alignItems: 'center', gap: 6 }}>
                <span>🔑</span>
                <span>Salva no navegador. Obtenha gratuitamente em </span>
                <a href="https://console.groq.com" target="_blank" rel="noreferrer" style={{ color: '#00BFFF', textDecoration: 'none' }}>console.groq.com</a>
              </div>
            </div>

            {/* Idioma */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 11, color: '#a78bfa', fontWeight: 700 }}>Idioma do vídeo</label>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {[['pt','🇧🇷 Português'],['en','🇺🇸 Inglês'],['es','🇪🇸 Espanhol'],['auto','🌐 Automático']].map(([val, label]) => (
                  <button key={val} onClick={() => !subtitleLoading && setSubtitleLang(val)}
                    style={{
                      padding: '6px 14px', borderRadius: 9, fontSize: 11, fontWeight: 700, cursor: subtitleLoading ? 'not-allowed' : 'pointer',
                      background: subtitleLang === val ? 'rgba(139,92,246,0.25)' : 'rgba(255,255,255,0.04)',
                      border: `1px solid ${subtitleLang === val ? 'rgba(139,92,246,0.7)' : 'rgba(255,255,255,0.08)'}`,
                      color: subtitleLang === val ? '#c4b5fd' : '#666',
                    }}>
                    {label}
                  </button>
                ))}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 11, color: '#a78bfa', fontWeight: 700 }}>
                    {subtitleProgress < 20 ? '🔊 Extraindo áudio...' :
                     subtitleProgress < 50 ? '🔄 Convertendo para WAV...' :
                     subtitleProgress < 85 ? '☁️ Enviando ao Whisper...' :
                     subtitleProgress < 98 ? '✍️ Gerando legendas...' : '✅ Concluído!'}
                  </span>
                  <span style={{ fontSize: 11, color: '#a78bfa', fontWeight: 700 }}>{subtitleProgress}%</span>
                </div>
                <div style={{ height: 8, background: 'rgba(255,255,255,0.06)', borderRadius: 999, overflow: 'hidden' }}>
                  <div style={{ height: '100%', borderRadius: 999, background: 'linear-gradient(90deg,#7b2ff7,#00BFFF)', width: `${subtitleProgress}%`, transition: 'width 0.4s ease' }} />
                </div>
              </div>
            )}
            ...
            {/* Erro */}
            {subtitleError && (
              <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 10, padding: '10px 14px', fontSize: 12, color: '#fca5a5', display: 'flex', alignItems: 'flex-start', gap: 8 }}>
            ...
            {!subtitleLoading && !subtitleError && (
              <div style={{ background: 'rgba(0,191,255,0.05)', border: '1px solid rgba(0,191,255,0.15)', borderRadius: 10, padding: '10px 14px', fontSize: 11, color: '#64748b', lineHeight: 1.6 }}>
                <strong style={{ color: '#00BFFF' }}>Como funciona:</strong> O áudio do vídeo é extraído localmente, convertido para WAV e enviado ao Groq Whisper. Os segmentos retornados são injetados automaticamente como legendas sincronizadas na timeline — prontas para editar fonte, cor, animação e posição.
              </div>
            )}
            ...
            {!subtitleLoading && (
              <button
                onClick={() => generateSubtitles(subtitleVideoId)}
                disabled={!subtitleApiKey.trim()}
                style={{
                  padding: '14px 0', borderRadius: 14, fontSize: 14, fontWeight: 800,
                  background: subtitleApiKey.trim() ? 'linear-gradient(135deg,#7b2ff7,#00BFFF)' : 'rgba(255,255,255,0.05)',
                  border: 'none', color: subtitleApiKey.trim() ? '#fff' : '#333',
                  cursor: subtitleApiKey.trim() ? 'pointer' : 'not-allowed',
                  boxShadow: subtitleApiKey.trim() ? '0 8px 24px rgba(123,47,247,0.35)' : 'none',
                  transition: 'all 0.2s',
                }}
              >
                🎤 Gerar Legendas
