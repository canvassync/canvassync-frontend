...
function ProModal({ onClose }) {
  return createPortal(
    <div onClick={onClose} style={{ position:'fixed', inset:0, zIndex:999999, background:'rgba(0,0,0,0.75)', display:'flex', alignItems:'center', justifyContent:'center', backdropFilter:'blur(6px)' }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'linear-gradient(135deg,#0d1117,#0a0f1e)', border:'1px solid rgba(167,139,250,0.4)', borderRadius:24, padding:'32px 28px', maxWidth:380, width:'90%', boxShadow:'0 32px 80px rgba(0,0,0,0.9)', display:'flex', flexDirection:'column', alignItems:'center', gap:16, textAlign:'center' }}>
...
<div style={{height:1,background:'rgba(255,255,255,0.06)',margin:'4px 0'}} />
                    ...
                    return (
                      ...
                      >
                        ...
                        <button onClick={()=>setShowProModal(true)}
                          style={{ margin:'4px 12px 12px', padding:'7px 0', borderRadius:9, cursor:'pointer', background:'rgba(167,139,250,0.1)', border:'1px solid rgba(167,139,250,0.3)', color:'#a78bfa', fontWeight:800, fontSize:12 }}
                          onMouseEnter={e=>{e.currentTarget.style.background='rgba(167,139,250,0.25)';}}
                          onMouseLeave={e=>{e.currentTarget.style.background='rgba(167,139,250,0.1)';}}
                        >🔒 PRO — {t('tpl_use')}</button>
                      </div>
                    );
                  })}
                </div>
                ...
                <div style={{ overflowY:'auto', flex:1, padding:'12px 12px 16px' }}>
                  ...
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8, opacity:0.4, pointerEvents:'none' }}>
                      ...
                      {OVERLAY_EFFECTS.slice(0,4).map(ov => (
                        <button key={ov.id} style={{ fontSize: 11, padding: '4px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)', background:'rgba(255,255,255,0.04)', color:'#888', cursor:'not-allowed', display:'flex', alignItems:'center', gap:4 }}>
                          <span>{ov.icon}</span>{ov.name}
                        </button>
                      ))}
                      <button onClick={()=>{ setShowFxPanel(false); setShowProModal(true); }} style={{ fontSize:11, padding:'4px 10px', borderRadius:8, border:'1px solid rgba(167,139,250,0.4)', background:'rgba(167,139,250,0.12)', color:'#a78bfa', cursor:'pointer', pointerEvents:'all', opacity:1, fontWeight:700 }}>🔒 Ver todos (PRO)</button>
                    </div>
                  </div>
                  <hr style={{ border: 'none', borderTop: '1px solid rgba(255,255,255,0.06)', margin: '0 0 12px' }} />
                  {FX_CATS.map(cat => {
                    const isPopular = cat.cat === '✨ Populares';
                    const freeIds = ['none','vignette','film_grain'];
                    return (
                    <div key={cat.cat} style={{ marginBottom:16 }}>
                      ...
                      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:7 }}>
                        {cat.items.map(fx => {
                          const isFree = isPopular && freeIds.includes(fx.id);
                          ...
                          if (isFree) {
                            ...
                            return (
                              <div key={fx.id} onClick={()=>{ setShowFxPanel(false); setShowProModal(true); }}
                                style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:5, padding:'10px 6px 8px', borderRadius:12, cursor:'pointer', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)', opacity:0.45, position:'relative', transition:'opacity 0.15s' }}
                                onMouseEnter={e=>e.currentTarget.style.opacity='0.7'}
                                onMouseLeave={e=>e.currentTarget.style.opacity='0.45'}
                              >
                                <div style={{ width:'100%', height:44, borderRadius:8, background:fx.preview||'#111', marginBottom:2, overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center', fontSize:24, border:'1px solid rgba(255,255,255,0.04)', filter:'grayscale(0.7)' }}>{fx.icon}</div>
                                <span style={{ fontSize:10, color:'#555', textAlign:'center', lineHeight:1.2 }}>{fx.label}</span>
                                <div style={{ position:'absolute', top:4, right:4, fontSize:8, color:'#a78bfa', background:'rgba(167,139,250,0.2)', borderRadius:4, padding:'1px 4px', fontWeight:700 }}>PRO</div>
                              </div>
                            );
                          }
                        })}
            ...
            <>
              ...
              <div style={{ position:'fixed', top:(exportBtnRef.current?.getBoundingClientRect().bottom??52)+4, right:Math.max(8,window.innerWidth-(exportBtnRef.current?.getBoundingClientRect().right??100)), zIndex:99998, background:'#0f172a', border:'1px solid rgba(0,191,255,0.2)', borderRadius:14, width:260, boxShadow:'0 16px 48px rgba(0,0,0,0.85)', overflow:'hidden' }}>
                ...
                <div style={{padding:'8px 10px', display:'flex', flexDirection:'column', gap:4}}>
                  {[
                    {value:'webm_offline_audio', label:'🎬 WebM com Áudio', desc:'', locked:true},
                    {value:'mp4',                label:'🎬 MP4 HD',          desc:'Alta qualidade', locked:true},
                    {value:'webm_hd',            label:'✨ WebM HD+',        desc:'Alta qualidade', locked:true},
                    {value:'mp4_hd',             label:'✨ MP4 HD+',         desc:'Padrão', locked:true},
                    {value:'png',                label:'🖼️ PNG',             desc:'Imagem estática', locked:false},
                    {value:'jpg',                label:'🖼️ JPG',             desc:'Imagem estática', locked:false},
                  ].map(fmt=>(
                    <div key={fmt.value}
                      onClick={()=>{ if(fmt.locked){ setShowExportPanel(false); setShowProModal(true); } else setExportFormat(fmt.value); }}
                      style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'7px 10px', borderRadius:8, cursor: fmt.locked?'pointer':'pointer', background: fmt.locked?'transparent': exportFormat===fmt.value?'rgba(0,191,255,0.12)':'transparent', border:`1px solid ${!fmt.locked&&exportFormat===fmt.value?'rgba(0,191,255,0.35)':'transparent'}`, transition:'all 0.12s', opacity: fmt.locked?0.45:1 }}
                      onMouseEnter={e=>{ e.currentTarget.style.background = fmt.locked?'rgba(167,139,250,0.06)':exportFormat!==fmt.value?'rgba(255,255,255,0.04)':e.currentTarget.style.background; }}
                      onMouseLeave={e=>{ e.currentTarget.style.background = !fmt.locked&&exportFormat===fmt.value?'rgba(0,191,255,0.12)':'transparent'; }}
                    >
            ...
            <div style={{
              ...
              <div style={{overflowY:'auto', flex:1, padding:'4px 0'}}>
                {(() => {
                  ...
                  return filtered.map((track, trackIdx) => {
                    ...
                    return (
                      ...
                      >
                        {/* Play/Pause */}
                        <button onClick={()=>{ if(isLocked) return; toggleTrilhasPreview(track); }}
                          style={{
                        ...
                        {/* Botão Usar / PRO */}
                        {isLocked ? (
                          <button onClick={()=>{ setShowTrilhasPanel(false); setShowProModal(true); }}
                            style={{ padding:'5px 12px', borderRadius:7, border:'1px solid rgba(167,139,250,0.4)', background:'rgba(167,139,250,0.12)', color:'#a78bfa', fontSize:11, cursor:'pointer', fontWeight:700, flexShrink:0 }}>
                            🔒 PRO
                          </button>
                        ) : (
                        <button
      ...
      <div style={{ display: 'flex', flex: 1, width: '100%', overflow: 'hidden' }}>
        ...
        <div className="cs-left-panel" style={{ width: '580px', minWidth: '580px', borderRight: '1px solid rgba(255,255,255,0.07)', display: 'flex', flexDirection: 'column', background: '#0d0d0d', boxShadow: 'none', overflowY: 'auto' }}>
                ...
                {(() => {
                  ...
                  return (
                    <div style={{ position:'relative', display:'flex', flexDirection:'column', gap:8, background:`${accentBg}0.04)`, border:`1px solid ${accentBg}0.18)`, borderRadius:12, padding:'10px 12px', overflow:'hidden' }}>
                      ...
                      <div style={{ display:'flex', gap:4, flexWrap:'wrap', filter:'blur(2px)', pointerEvents:'none', userSelect:'none' }}>
                        {['—','Fade','Slide ↑','Zoom +','Blur','Bounce','Rolar','Pulsar'].map(l=>(
                          <span key={l} style={{ padding:'3px 8px', fontSize:10, borderRadius:7, background:`${accentBg}0.08)`, border:`1px solid ${accentBg}0.2)`, color: accent }}>{l}</span>
                        ))}
                      </div>
                    </div>
                  );
          ...
          {/* ══ SEÇÃO KEYFRAMES + MÁSCARAS + ABERRAÇÃO CROMÁTICA — BLOQUEADO no Free ══ */}
          {(activeImageId || activeVideoId) && (() => {
            const selImg = activeImageId ? images.find(i => i.id === activeImageId) : null;
            const selVid = activeVideoId ? videos.find(v => v.id === activeVideoId) : null;
            const sel    = selImg || selVid;
            ...
            return (
              <div style={{ padding: '12px 18px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                <div onClick={()=>setShowProModal(true)} style={{ position:'relative', borderRadius:12, border:`1px solid ${accentBg}0.18)`, background:`${accentBg}0.03)`, padding:'14px 16px', cursor:'pointer', display:'flex', flexDirection:'column', gap:8, overflow:'hidden' }}>
                  ...
                  <div style={{ display:'flex', gap:4, flexWrap:'wrap', filter:'blur(2px)', pointerEvents:'none', userSelect:'none' }}>
                    {['Sem','⬤ Círculo','◇ Losango','★ Estrela','♥ Coração'].map(l=>(
                      <span key={l} style={{ padding:'3px 8px', fontSize:10, borderRadius:8, background:`${accentBg}0.08)`, border:`1px solid ${accentBg}0.15)`, color:'#555' }}>{l}</span>
                    ))}
                  </div>
                  <span style={{ fontSize:11, color: accent, fontWeight:700, filter:'blur(1px)' }}>⚡ Aberração Cromática · 🔍 Zoom Animado</span>
                </div>
              </div>
            );
          })()}
          {/* ══ BLOCO ANTIGO REMOVIDO — Máscara/Aberração/Zoom agora são PRO ══ */}
          {false && (
              <div>
                {/* ── Máscara de Forma ── */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <div style={{ padding: '14px 18px 12px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
              <textarea
                ...
                style={{ flex: 1, padding: '10px 12px', backgroundColor: extraTexts.length >= 1 ? '#0a0a0a' : '#111', border: '1px solid rgba(255,255,255,0.08)', color: extraTexts.length >= 1 ? '#444' : '#f0f0f0', borderRadius: '14px', fontSize: '12px', resize: 'none', lineHeight: '1.5', fontFamily: 'inherit', cursor: extraTexts.length >= 1 ? 'not-allowed' : 'text' }}
              />
              {extraTexts.length >= 1 ? (
                <button onClick={()=>setShowProModal(true)} style={{ padding: '0 12px', height: '56px', background: 'rgba(167,139,250,0.12)', border: '1px solid rgba(167,139,250,0.3)', borderRadius: '14px', cursor: 'pointer', fontWeight: 'bold', color: '#a78bfa', fontSize: '11px', whiteSpace:'nowrap' }}>🔒 PRO</button>
              ) : (
            ...
            {extraTexts.length >= 1 && (
              <div style={{ fontSize: 10, color: '#a78bfa', background: 'rgba(167,139,250,0.08)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: 8, padding: '5px 10px', display:'flex', alignItems:'center', gap:6 }}>
                🔒 Limite do plano gratuito: 1 texto extra. <span onClick={()=>setShowProModal(true)} style={{ cursor:'pointer', textDecoration:'underline', fontWeight:700 }}>Ativar Pro</span>
              </div>
            )}
          ...
          <div style={{ flex: 1, padding: '14px 18px 14px', display: 'flex', flexDirection: 'column', gap: '10px', minHeight: 0 }}>
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'stretch' }}>
              ...
              <button
                onClick={()=>{ if(lyrics.length >= 3){ setShowProModal(true); return; } markLyricTiming(); }}
                disabled={textLines.length === 0 || currentLineIndex >= textLines.length}
                style={{
                  padding: '0 20px',
                  minWidth: '110px',
                  background: lyrics.length >= 3
                    ? 'rgba(167,139,250,0.12)'
                    : (textLines.length > 0 && currentLineIndex < textLines.length)
                      ? '#00BFFF'
                      : 'rgba(255,255,255,0.04)',
                  color: lyrics.length >= 3 ? '#a78bfa' : (textLines.length > 0 && currentLineIndex < textLines.length) ? '#000' : '#444',
                  border: lyrics.length >= 3 ? '1px solid rgba(167,139,250,0.4)' : '1px solid rgba(255,255,255,0.08)',
                  borderRadius: '16px',
                  fontWeight: 'bold',
                  cursor: lyrics.length >= 3 ? 'pointer' : (textLines.length > 0 && currentLineIndex < textLines.length) ? 'pointer' : 'not-allowed',
                  fontSize: '12px',
                  lineHeight: 1.4,
                  textAlign: 'center',
                  flexShrink: 0,
                  boxShadow: (textLines.length > 0 && currentLineIndex < textLines.length && lyrics.length < 3) ? '0 4px 18px rgba(0,191,255,0.3)' : 'none',
                  transition: 'all 0.2s',
                }}
              >
                {lyrics.length >= 3 ? '🔒 PRO' : textLines.length === 0 ? t('ed_paste_lyrics_short') : currentLineIndex >= textLines.length ? t('ed_all_done') : t('ed_mark_now')}
              </button>
            </div>
            ...
            {lyrics.length >= 3 && (
              <div style={{ fontSize: 10, color: '#a78bfa', background: 'rgba(167,139,250,0.08)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: 8, padding: '5px 10px', display:'flex', alignItems:'center', gap:6 }}>
                🔒 Limite do plano gratuito: 3 frases. <span onClick={()=>setShowProModal(true)} style={{ cursor:'pointer', textDecoration:'underline', fontWeight:700 }}>Ativar Pro</span>
              </div>
            )}

            {/* Botão Sincronização Automática IA — BLOQUEADO no Free */}
            <button
              ref={syncBtnRef}
              onClick={() => setShowProModal(true)}
