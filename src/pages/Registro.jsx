...
export default function Registro() {
  ...
  const card = { width: '100%', maxWidth: 460, background: '#0d0d0d', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 24, padding: '36px 32px' };
    ...
    <div style={page}>
      ...
      <div style={card}>
        ...
        <h1 style={{ fontSize: 22, fontWeight: 800, marginBottom: 6, textAlign: 'center' }}>
          {authMode === 'register' ? t('reg_title_register') : t('reg_title_login')}
        </h1>
        <p style={{ fontSize: 13, color: '#444', textAlign: 'center', marginBottom: 24 }}>
          {authMode === 'register' ? t('reg_sub_register') : t('reg_sub_login')}
        </p>

        <div style={{ display: 'flex', background: '#111', borderRadius: 12, padding: 4, marginBottom: 24 }}>
          {[['register', t('register_tab')], ['login', t('signin_tab')]].map(([mode, label]) => (
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <button type="submit" disabled={loading} style={{ ...btn, opacity: loading ? 0.6 : 1, cursor: loading ? 'not-allowed' : 'pointer', marginTop: 4 }}>
            {loading ? t('loading_btn') : authMode === 'register' ? t('reg_create_btn') : t('reg_login_btn')}
          </button>
        </form>

        <div style={{ marginTop: 24, background: '#111', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '14px 16px' }}>
          <p style={{ fontSize: 11, color: '#444', fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 10 }}>{t('free_includes')}</p>
          {[t('reg_free1'), t('reg_free2'), t('reg_free3'), t('reg_free4')].map((item, i) => (
        ...
        <p style={{ textAlign: 'center', fontSize: 12, color: '#333', marginTop: 20 }}>
          {t('want_pro')}{' '}
          <button onClick={() => window.location.href = '/planos'} style={{ background: 'none', border: 'none', color: '#00BFFF', cursor: 'pointer', fontWeight: 700, fontSize: 12, padding: 0 }}>
            {t('see_pro')}
          </button>
        </p>
      </div>

      <button onClick={() => window.location.href = '/'} style={{ marginTop: 20, background: 'none', border: 'none', color: '#333', fontSize: 13, cursor: 'pointer' }}>
        {t('back_home')}
      </button>
    </div>
  );
